...
export default class TurmsBusinessError extends Error {
    private readonly _isTurmsBusinessError;
    ...
    static notFalsy(name: string, notEmpty?: boolean): never;
    static illegalParamPromise<T = never>(reason: string): Promise<T>;
    static notFalsyPromise<T = never>(name: string, notEmpty?: boolean): Promise<T>;
}
