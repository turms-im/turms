...
function createBaseUpdateGroupJoinRequestRequest(): UpdateGroupJoinRequestRequest {
  return { requestId: "0", responseAction: 0, reason: undefined };
}

export const UpdateGroupJoinRequestRequest = {
  encode(message: UpdateGroupJoinRequestRequest, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): UpdateGroupJoinRequestRequest {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseUpdateGroupJoinRequestRequest();
