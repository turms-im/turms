...
export interface DeleteMessageReactionsRequest {
  messageId: string;
  reactionTypes: number[];
  customAttributes: Value[];
}

function createBaseDeleteMessageReactionsRequest(): DeleteMessageReactionsRequest {
  return { messageId: "0", reactionTypes: [], customAttributes: [] };
}

export const DeleteMessageReactionsRequest: MessageFns<DeleteMessageReactionsRequest> = {
  encode(message: DeleteMessageReactionsRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): DeleteMessageReactionsRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseDeleteMessageReactionsRequest();
