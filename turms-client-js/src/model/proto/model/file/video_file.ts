...
export const VideoFile: MessageFns<VideoFile> = {
  encode(message: VideoFile, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.description !== undefined) {
      VideoFile_Description.encode(message.description, writer.uint32(10).fork()).join();
    }
...
export const VideoFile_Description: MessageFns<VideoFile_Description> = {
  encode(message: VideoFile_Description, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.url !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): VideoFile_Description {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
