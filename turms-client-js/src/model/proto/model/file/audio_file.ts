...
export const AudioFile: MessageFns<AudioFile> = {
  encode(message: AudioFile, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.description !== undefined) {
      AudioFile_Description.encode(message.description, writer.uint32(10).fork()).join();
    }
...
export const AudioFile_Description: MessageFns<AudioFile_Description> = {
  encode(message: AudioFile_Description, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.url !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): AudioFile_Description {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
