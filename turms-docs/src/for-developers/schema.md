...
* 业务场景：是否需要支持消息发送人能够查询他自己发送的消息
  ...
  * 方案二：支持该特性，使用`消息发送时间 + 会话ID`复合索引
    ...
    补充知识：`会话ID`是16字节长的字节数组，其值由消息发送者ID、消息接收者/群ID与该消息是否是群消息这三个元素决定。会话ID的低位由值小的8字节ID组成、高位由值大的8字节ID组成。其中，由于ID自身永远是正数，即ID的最高有效位永远是0，因此在会话ID中，高位ID的最高有效位被用于区分该会话ID是私聊会话，还是群聊会话。如果是0那就是私聊会话，是1就是群聊会话，以避免会话ID冲突。
