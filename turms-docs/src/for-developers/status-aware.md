...
总的来说，Turms架构基于读扩散消息模型设计，未来可能会辅之以写扩散做配合（即私聊与小群用写扩散，大群用读扩散）。
...
| 优势场景   | 用户会话（私聊会话与群里会话）相对少，群人数多的场景         | 适合私聊会话相对多，但群会话少且群人数也少的场景             |
| 劣势场景   | 用户的会话数量，且读的频繁                                   | 单个群的用户数多，且发送消息频发                             |
| 技术实现   | 可以通过MongoDB的分片副本架构，对读请求进行负载均衡          | 1. 写操作难以进行负载均衡<br />2. 更新消息、撤回消息等功能实现成本巨大（分布式一致性问题与消息风暴） |
| 消息可靠性 | 如产品对消息可靠性有较高的要求（保证消息不丢，保证消息内容一致），读扩散对应的实现相对简单，且性能比写扩散好得多 | 为保证消息写入到每位群成员的信箱中，需要引入弱分布式一致性事务（或强分布式一致性事务），性能低下 |
| 总评       | 适用读扩散的产品极广，对于写扩散实现成本巨大的实现，读扩散通常都只需要一条查询语句即可解决（如群组新成员消息分享、多端消息同步），服务端甚至不需要改一行代码。<br />并且，对比读扩散与写扩散各自的劣势场景，读扩散在劣势场景下依然能保证较高的效率 | 由于写扩散需要写入大量的消息，如有任何更新操作（撤回/更新）还需要使用分布式事务、群组新成员消息分享、多端同步实现复杂等等，<br />因此写扩散的业务拓展性极差，其使用场景基本限定在：应用基本都是私聊，没有群聊，且业务功能简单。<br />如果产品经理要求添加业务功能，您的开发团队很快就会体会到只支持写扩散对IM系统是多么致命的设计。只要业务需求超出上述范围，基本上用了写扩散模型就意味着未来要重构回读扩散模型，或者同时支持读扩散与写扩散（同时维护两套模型，最终逻辑会非常复杂） |

再次特别强调：除非您非常明确您的产品的用例就如上述简单，且未来业务需求也基本不变，否则基本上用了写扩散模型就意味着您的产品终有一天需要重构回读扩散模型，或同时支持读写两套模型。

提醒：“撤回消息”在实现上也是一条消息，即一种特殊的系统消息。
...
特大群的实现其实并不难，只是它的需求与场景跟一般社交应用的很不一样，所以要有一套专门的策略来支持特大群。
