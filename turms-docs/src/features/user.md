...
| 用户权限组           | 管理员可以针对不同的用户给予不同的权限                       | im.turms.turms.pojo.domain.UserPermissionGroup |
...
| 附近的人       | 各Turms服务端基于R*-tree算法管理与自身连接的在线用户的地理坐标 | 根据当前实时坐标搜寻附近的其他用户           | Location.maxAvailableUsersNearbyNumberForQuery<br />         |
| 在线用户数统计 | 只有Turms集群的Master节点会定期进行在线用户数的统计工作      | 定时将当前集群所连接的用户数量记录到数据库中 | logOnlineUsersNumberIntervalSeconds                          |
