...
领域模型：im.turms.turms.pojo.domain.GroupType
...
面向管理员的接口：请参考[Turms的Swagger文档](https://github.com/turms-im/turms/blob/develop/turms/docs/html/swagger.html)

面向用户的接口：请参考turms/src/main/proto/request/group目录下的接口描述文件

群成员类型包括：群主、管理员、普通成员、游客、匿名游客

配置属性类：im.turms.turms.property.business.Group
...
| 发送入群请求         |                                                              | groupJoinRequestContentLimit<br />groupJoinRequestTimeToLiveHours |
