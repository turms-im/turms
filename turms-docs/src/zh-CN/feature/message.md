...
默认配置下，Turms不支持“在私聊会话中，消息发送者能够查询他自己发送的消息”（具体原因：[消息索引设计](https://turms-im.github.io/docs/zh-CN/design/schema.html#%E7%B4%A2%E5%BC%95)。注意：在群聊会话中，消息发送者始终能够查询他自己发送的消息），开发者可以通过在turms-service服务端的配置文件中配置`turms.service.message.use-conversation-id=true`来启用`会话ID`。
