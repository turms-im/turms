...
* **集合索引不针对数据分析做设计**（具体请查阅 [Turms数据分析](https://turms-im.github.io/docs/zh-CN/server/module/data-analytics.html)）
...
不少集合的复合索引都带上了`记录创建时间`字段，这是为了配合Turms的[拉模式](https://turms-im.github.io/docs/zh-CN/design/status-aware.html#%E6%84%9F%E7%9F%A5%E6%96%B9%E5%BC%8F)，以支持快速查询某时间区间的记录，并避免客户端重复查询。这也是为什么Turms客户端大部分查询语句都可以带上一个查询时间区间的参数，而如果客户端请求没带上这参数，那么Turms服务端就会默认赋予一个查询时间区间，以保证查询性能。
