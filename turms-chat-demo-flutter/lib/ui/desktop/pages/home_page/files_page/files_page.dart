...
class _FilesPageState extends ConsumerState {
  ...
  void initState() {
    super.initState();
    TAsyncData.fromFuture(
      () => ref.read(fileServiceProvider)!.queryFiles(),
    ).forEach((data) {
      if (mounted) {
        _fileInfosData = data;
        setState(() {});
        ...
        header: TTableRow(
          cells: [
            const TTableDataCell(
              widget: Icon(Symbols.insert_drive_file_rounded),
            ),
            ...
            .map(
              (e) => TTableRow(
                ...
                cells: [
                  ...
                  _buildTextDataCell(
                    appThemeExtension,
                    dateFormat.format(e.uploadDate),
                    true,
                  ),
                  _buildTextDataCell(appThemeExtension, e.uploader, true),
                  _buildTextDataCell(
                    appThemeExtension,
                    e.size.toHumanReadableFileSize(),
                    true,
                  ),
                  const TTableDataCell(
                    widget: Padding(
