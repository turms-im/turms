...
public class AssertUtil {
    ...
    public static void notNull(Object object, String name) {
        if (object == null) {
            throw TurmsBusinessException.get(TurmsStatusCode.ILLEGAL_ARGUMENTS, name + " must not be null");
        }
    }
    ...
    public static void length(String s, String name, int min, int max) {
        if (s != null) {
            ...
            if (length < min || length > max) {
                throw TurmsBusinessException.get(TurmsStatusCode.ILLEGAL_ARGUMENTS, name + " must have length between " + min + " and " + max);
            }
        }
    }
    ...
    public static void maxLength(String s, String name, int max) {
        if (s != null) {
            int length = s.length();
            if (length > max) {
                throw TurmsBusinessException.get(TurmsStatusCode.ILLEGAL_ARGUMENTS, name + " must have length less than " + max);
            }
        }
    }
    ...
    public static void noWhitespace(String s, String name) {
        if (StringUtils.containsWhitespace(s)) {
            throw TurmsBusinessException.get(TurmsStatusCode.ILLEGAL_ARGUMENTS, name + " must not contain whitespace");
        }
    }
    ...
    public static void pastOrPresent(Date date, String name) {
        if (date != null && date.getTime() > System.currentTimeMillis()) {
            throw TurmsBusinessException.get(TurmsStatusCode.ILLEGAL_ARGUMENTS, name + " must be a date in the past or in the present");
        }
    }

    public static void before(Date startDate, Date endDate, String startDateName, String endDateName) {
        if (startDate != null && endDate != null && endDate.before(startDate)) {
            throw TurmsBusinessException.get(TurmsStatusCode.ILLEGAL_ARGUMENTS, endDateName + " must not be before " + startDateName);
        }
    }
