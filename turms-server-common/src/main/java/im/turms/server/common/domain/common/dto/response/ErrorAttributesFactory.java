...
public final class ErrorAttributesFactory {
    ...
    public static ErrorAttributes parse(Throwable throwable) {
        ...
        if (throwable instanceof ResponseStatusException e) {
            ...
            attributes = SimpleErrorAttributes.fromResponseException(e);
        } else {
    ...
    private static record SimpleErrorAttributes(
        ...
        private static SimpleErrorAttributes fromResponseException(ResponseException exception) {
            ResponseStatusCode statusCode = exception.getCode();
        ...
        private static SimpleErrorAttributes fromResponseStatusException(ResponseStatusException exception) {
            ...
            String reason = exception.getReason();
            ResponseStatusCode statusCode = switch (httpStatus.series()) {
                case INFORMATIONAL, SUCCESSFUL, REDIRECTION -> ResponseStatusCode.OK;
                case CLIENT_ERROR -> ResponseStatusCode.INVALID_REQUEST;
                case SERVER_ERROR -> ResponseStatusCode.SERVER_INTERNAL_ERROR;
