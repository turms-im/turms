...
public class MongoEntity<T> {
    ...
    public MongoEntity(Class<T> clazz) {
        this.clazz = clazz;
        constructor = PreferredConstructorDiscoverer.discover(clazz);
        collectionName = parseCollectionName(clazz);
        shardKey = parseShardKey(clazz);
        compoundIndex = parseCompoundIndex(clazz);
        EntityFieldsInfo entityFieldsInfo = parseFields(clazz, constructor, clazz.getDeclaredMethods());
        indexes = entityFieldsInfo.getIndexes();
        idFieldName = entityFieldsInfo.getIdFieldName();
        fieldMap = entityFieldsInfo.getFields();
    }
    ...
    private int parseCtorParamIndex(PreferredConstructor<T, ?> constructor, Field field) {
        String fieldName = parseFieldName(field);
    ...
    public <T> EntityField<T> getField(String fieldName) {
        return (EntityField<T>) fieldMap.get(fieldName);
    }
