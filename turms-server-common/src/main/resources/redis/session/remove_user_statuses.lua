...
if device_count == 0 then
    ...
    KEYS[3] = string.char(0)
    KEYS[4] = string.char(1)
    KEYS[5] = string.char(2)
    KEYS[6] = string.char(3)
    KEYS[7] = string.char(4)
    KEYS[8] = string.char(5)
end

local node_ids = redis.call('HMGET', user_id, unpack(KEYS, 3, key_count))
...
if has_devices_to_delete then
    redis.call('HDEL', user_id, unpack(devices_to_delete))
end

local field_count = redis.call('HLEN', user_id)
if field_count <= 2 then
    redis.call('DEL', user_id)
else
    local has_related_device = false
    local values = redis.call('HVALS', user_id)
    ...
    if not has_related_device then
        redis.call('HDEL', user_id, node_id)
    end
end
