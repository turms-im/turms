         ...
         [weakThis = std::weak_ptr(shared_from_this()),
          ...
          connectTimeout]() mutable {
             ...
             sharedThis->resolver_.async_resolve(
                 host,
                 std::to_string(port),
                 [weakThis = std::weak_ptr(sharedThis),
                  promise,
                  connectTimeout,
...
auto TcpClient::close(const std::optional<std::exception>& e) -> boost::future<void> {
    boost::promise<void> promise;
    boost::future<void> future = promise.get_future();
    boost::asio::post(
        [weakThis = std::weak_ptr(shared_from_this()), e, promise = std::move(promise)]() mutable {
            const auto sharedThis = weakThis.lock();
            if (sharedThis == nullptr) {
                return;
            }
...
auto TcpClient::write(const std::shared_ptr<std::array<uint8_t, N>>& bytes) -> boost::future<void> {
    ...
    try {
        boost::asio::async_write(
            ...
            [weakThis = std::weak_ptr(shared_from_this()), promise = std::move(promise)](
                const boost::system::error_code& errorCode, std::size_t bytesTransferred) mutable {
                if (errorCode) {
                    auto e{boost::system::system_error(errorCode)};
...
auto TcpClient::writeVarIntLengthAndBytes(const std::shared_ptr<std::vector<uint8_t>>& bytes)
    ...
    boost::promise<void> promise;
    boost::future<void> future = promise.get_future();
    const uint32_t dataLength = bytes->size();
