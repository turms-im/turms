...
①turms服务端v0.8；②turms-admin；③turms-client-js；④turms-client-dart；⑤turms-client-flutter-demo
...
- 下载并解压Turms服务端压缩包
- 下载并解压CockRoachDB压缩包，将“cockroach.exe”文件存放到Turms服务端目录下的"/cockroachdb"文件夹下
- 运行Turms服务端可执行脚本
- 在多台服务器上重复以上步骤，Turms服务端将自动寻找其他Turms服务端节点，由此集群开始运作
...
注意：在Turms中，某用户的联系人既可是是好友，也可以是被拉黑用户。联系人仅用于表示该用户所拥有的“联系人(Contacts)”这一概念，不指代其间的其他关系，与微信中联系人概念类似。
...
- 此表内容尤其URL部分均为Preview用途，非最终定稿。实际定稿在实际发布后统一更新
- Turms面向管理员使用的API接口并不是完全按照RESTful风格设计的，其Turms接口的设计原则是“接口的规范性要以开发者使用便捷为前提”，而不是“为了接口规范，需要开发者进行多余的繁琐操作”

### 管理员权限管理

| **功能**       | **功能说明**                                                 | **URL**                | **实现版本** |
| :------------- | :----------------------------------------------------------- | ---------------------- | ------------ |
| 管理员登陆     |                                                              | POST /admins/session   | 0.8.0 ✔      |
| 管理员退出     |                                                              | DELETE /admins/session | 0.8.0 ✔      |
| 新建管理员     |                                                              | POST /admins           | 0.8.0 ✔      |
| 删除管理员     |                                                              | DELETE /admins         | 0.8.0 ✔      |
| 管理员权限控制 | 每个集群默认存在一个userId为“turms”，password为“turms”的管理员 |                        | 0.8.0 ✔      |
...
| **功能**           | **功能说明**                                                 | **URL**                  | **实现版本** |
| :----------------- | :----------------------------------------------------------- | ------------------------ | ------------ |
| 获取用户信息       | 获取用户的完整信息（所有字段信息）                           | GET /users               | 0.8.0 ✔      |
...
| 获取用户在线信息   | 获取用户在线信息，包括用户在线状态、用户位置（如果客户端有提供此信息）、用户在线设备类型与用户在线设备登陆时间 | GET /users/statuses      | 0.8.0 ✔      |
| 修改用户在线状态   | 修改用户在线状态，可用于强制用户下线。不可修改下线用户的在线状态 | PUT /users/statuses      | 0.8.0 ✔      |
| 获取用户附近的人   | 根据该用户的最新坐标，获取其附近的其他用户的ID（主要：此服务要求此用户客户端有提供用户坐标信息） | GET /users/people-nearby | 0.8.0 ✔      |
| 获取用户的历史坐标 | 获取用户的所有历史坐标，每个坐标信息由具体坐标位置与时间戳组成 | GET /users/locations     | 0.8.0 ✔      |

### 用户关系管理

提醒：“联系人”既可以是“好友”，也可以是“拉黑用户”

| **功能**             | **描述**                                   | **URL**                      | **实现版本** |
| :------------------- | :----------------------------------------- | :--------------------------- | ------------ |
| 获取某用户联系人名单 | 根据可选分组条件，获取某用户联系人ID       | GET /users/contacts          | 0.8.0 ✔      |
| 移除联系人           | 移除某用户联系人列表中的所有用户或指定用户 | DELETE /users/contacts       | 0.8.0 ✔      |
| 修改联系人关系       | 修改联系人关系。将关系变为“好友”或“用户”   | PUT /users/contacts          | 0.8.0 ✔      |
| 新建联系人分组       |                                            | POST /users/contacts-group   | 0.8.0 ✔      |
| 修改联系人分组       |                                            | PUT /users/contacts-group    | 0.8.0 ✔      |
| 删除联系人分组       |                                            | DELETE /users/contacts-group | 0.8.0 ✔      |
| 获取联系人所在分组   |                                            | GET /users/contacts/group    | 0.8.0 ✔      |
| 添加联系人至分组     |                                            | POST /users/contacts/group   | 0.8.0 ✔      |
| 将联系人移除分组     |                                            | DELETE /users/contacts/group | 0.8.0 ✔      |
| 获取好友名单         | 根据可选分组条件，获取某用户好友ID         | GET /users/friends           | 0.8.0 ✔      |
| 添加好友             | 添加至好友名单                             | POST /users/friends          | 0.8.0 ✔      |
| 移除好友             | 移除好友名单中的用户                       | DELETE /users/friends        | 0.8.0 ✔      |
| 获取拉黑用户名单     | 根据可选分组条件，获取某用户拉黑用户ID     | GET /users/blacklists        | 0.8.0 ✔      |
| 添加拉黑用户         | 添加至黑名单                               | POST /users/blacklists       | 0.8.0 ✔      |
| 移除拉黑用户         | 移除黑名单中的用户                         | DELETE /users/blacklists     | 0.8.0 ✔      |
...
| 消息推送             |                                                              |                                                | 1.x ❓        |
| 分布式动态配置       | 长期采用的过渡方案：基于Hazelcast的2PC协议与Hazelcast的ReplicatedMap实现 | 分布式动态配置                                 | 1.0 ❓        |
| 对话机器人           |                                                              | 对话机器人                                     | 2.x ❓        |
